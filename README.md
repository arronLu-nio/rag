...
> LangGraph 工作流编排，MySQL + Milvus + OpenSearch 混合检索，支持文档上传、版本更新、权限过滤、引用溯源和低分拒答。

这是一个可运行的企业知识库 RAG MVP。它不仅能把文本转向量后问答，也把文档全文、检索索引、关键词搜索、重排、引用和文档生命周期拆成了明确的职责边界。
...
```text
纯 Milvus 召回分数 < 0.2：直接拒答
rerank Top 1 分数 < 0.5：直接拒答
```

混合检索使用 RRF 分数，和余弦相似度不是同一量纲，因此最终是否回答以 rerank 阈值为准。
